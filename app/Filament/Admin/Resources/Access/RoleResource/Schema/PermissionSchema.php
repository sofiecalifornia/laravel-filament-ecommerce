...
namespace App\Filament\Admin\Resources\Access\RoleResource\Schema;

use App\Filament\Admin\Resources\Access\RoleResource\Support\PermissionData;
        ...
        return [
            ...
            Forms\Components\Hidden::make('permissions')
                ->afterStateHydrated(function (Forms\Components\Hidden $component, ?Role $record): void {
                    $component->state($record ? $record->permissions->pluck('uuid') : []);
                })
                ->dehydrateStateUsing(
                        ...
                        ->reduce(
                            function (
                                ...
                                if ($get($parentPermission) ?? false) {
                                    $uuid =
                                        $permissionGroup
                                            ->firstWhere('name', $parentPermission)
                                            ?->uuid
                                        ?? throw new Exception('Can not use uuid of null.');
                                    $result->push($uuid);
                                                ...
                                                ->afterStateHydrated(
                                                    function (Forms\Components\CheckboxList $component, ?Role $record) use (
                                                        ...
                                                        if ($record->hasPermissionTo($parentPermission)) {
                                                            ...
                                                            $component->state($permissionsDatas->pluck('uuid')->toArray());
    ...
    private static function permissions(): CollectionSupport
    {
        return once(
            fn () => app(PermissionRegistrar::class)
                ...
                ->sortBy('name')
                ->map(fn (Permission $permission): PermissionData => new PermissionData($permission->uuid, $permission->name))
        );
    }
    ...
    private static function parentAbilitiesWithIdAndLabels(CollectionSupport $permissionsDatas): CollectionSupport
    {
        return self::permissionsExceptParent($permissionsDatas)
            ->mapWithKeys(fn (PermissionData $permissionData) => [
                $permissionData->uuid => Str::headline($permissionData->child_name ?? throw new Exception('Can not use $permissionData->child_name when null return.')),
            ]);
    }
